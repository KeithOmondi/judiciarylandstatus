...
const FilePreviewModal = ({ url, fileName, onClose }: PreviewModalProps) => {
  ...
  useEffect(() => {
    ...
    // Skip fetch if this exact URL is already loaded
    if (url === lastFetchedUrl.current && blobUrl) return;

    lastFetchedUrl.current = url;
    dispatch(streamFile(url));

    return () => {
      // The slice's pending case already revokes the previous blob URL,
      // and revokeBlobUrl handles cleanup on unmount
