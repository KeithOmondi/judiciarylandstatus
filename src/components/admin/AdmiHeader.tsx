...
export const AdminHeader: React.FC<AdminHeaderProps> = ({ onMenuToggle }) => {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-stone-200 bg-white px-4 lg:px-8 shadow-sm">
      {/* Mobile Hamburger & Badge Controls */}
      ...
      <div className="flex items-center gap-3 lg:gap-6">
        <div className="flex items-center gap-2 lg:gap-3 border-r border-stone-200 pr-3 lg:pr-6 text-right">
          ...
          <div className="flex h-8 w-8 lg:h-9 lg:w-9 shrink-0 items-center justify-center rounded-full bg-stone-100 text-xs lg:text-sm font-bold text-stone-700 ring-1 ring-stone-200">
            {user?.full_name ? user.full_name.split(' ').map(n => n[0]).join('') : 'AD'}
          </div>
        </div>
